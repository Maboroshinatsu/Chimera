...
export async function loadModel(modelPath: string, app: Application): Promise<Live2DModel<Cubism4ModelSettings>> {
...
export function resizeModel(
  ...
  cw: number, ch: number, zoom: number = 1.0,
  fx?: number, fy?: number,
) {
...
export function resizeModelFit(
  ...
  const base = Math.min((cw * 0.85) / modelRefW, (ch * 0.85) / modelRefH)
  model.scale.set(Math.max(0.01, Math.min(30.0, base * zoom)))
  model.position.set(cw / 2, ch / 2)
}
